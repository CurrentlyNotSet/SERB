 ...
 * @author Andrew
 */
public class AppealCourtAddEditDialog extends javax.swing.JDialog {

    private int ID;
    private AppealCourt item;
     ...
     * @param itemIDpassed
     */
    public AppealCourtAddEditDialog(java.awt.Frame parent, boolean modal, int itemIDpassed) {
        super(parent, modal);
        initComponents();
    ...
    private void loadCategories(){
        ...
        categoryComboBox.addItem("CO");
        categoryComboBox.addItem("DC");
        categoryComboBox.addItem("ST");
    }
    
    private void loadInformation() {
        item = AppealCourt.getAppealCourtByID(ID);
        
        if (item.type != null){
            categoryComboBox.setSelectedItem(item.type);
        }
        DescriptionTextField.setText(item.courtName);
    }
    
    private void saveInformation() {
        item.id = ID;
        item.type = categoryComboBox.getSelectedItem().toString().trim();
        item.courtName = DescriptionTextField.getText().trim();
                       
        if (ID > 0){
            AppealCourt.updateAppealCourt(item);
        } else {
            AppealCourt.createAppealCourt(item);
        }
    }
