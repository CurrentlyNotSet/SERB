 ...
 * @author Andrew
 */
public class RetentionExclusionAddEditDialog extends javax.swing.JDialog {

    private int ID;
    private RetentionExclusion item;
     ...
     * @param itemIDpassed
     */
    public RetentionExclusionAddEditDialog(java.awt.Frame parent, boolean modal, int itemIDpassed) {
        super(parent, modal);
        initComponents();
    ...
    private void setDefaults(int itemIDpassed) {
        ...
        loadCategories();
        if (ID > 0) {
            titleLabel.setText("Edit Retention Exclude Text");
            editButton.setText("Save");
            loadInformation();
        } else {
            titleLabel.setText("Add Retention Exclude Text");
            editButton.setText("Add");
            editButton.setEnabled(false);
            item = new RetentionExclusion();
        }
    ...
    private void loadInformation() {
        item = RetentionExclusion.getRetentionExclusionByID(ID);
        
        if (item.section != null){
            SectionComboBox.setSelectedItem(item.section);
        }
        DescriptionTextField.setText(item.excludeString);
    }
    
    private void saveInformation() {
        item.id = ID;
        item.section = SectionComboBox.getSelectedItem().toString().trim();
        item.excludeString = DescriptionTextField.getText().trim();
                       
        if (ID > 0){
            RetentionExclusion.updateRetentionExclusion(item);
        } else {
            RetentionExclusion.createRetentionExclusion(item);
        }
    }
